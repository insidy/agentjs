...
public class AgentScriptManager {
	...
	public ArrayList<AgentScript> getLocalScripts() {
		...
		newScript
				.setSourceCode("var log = Packages.br.unisinos.swe.agentjs.engine.EngineLogger.i; var notif = agent.createNotification('teste'); notif.content = 'opa!'; notif.send(); log('pronto');");

		scripts.add(newScript);

		newScript = new AgentScript();
		newScript
				.setSourceCode("var log = Packages.br.unisinos.swe.agentjs.engine.EngineLogger.i; agent.sms.send({ destination: '5556', message: 'Ola!'}, function(err) { log(err); });");

		scripts.add(newScript);

		newScript = new AgentScript();
		newScript
				.setSourceCode("var log = Packages.br.unisinos.swe.agentjs.engine.EngineLogger.i; log('music: ' + agent.music); agent.music.playFromUrl('/Music/Chasing The Sun.mp3');");

		scripts.add(newScript);

		newScript = new AgentScript();
		newScript
				.setSourceCode("var log = Packages.br.unisinos.swe.agentjs.engine.EngineLogger.i; log('network: ' + agent.network); agent.network.on('wifi:connected', function(data) { log('conectou'); });");

		scripts.add(newScript);

		newScript = new AgentScript();
		newScript
				.setSourceCode("var log = Packages.br.unisinos.swe.agentjs.engine.EngineLogger.i; log('application: ' + agent.apps); var installedApps = agent.apps.getInstalledApps(); for(var i = 0; i < installedApps.length; i++){ log(installedApps[i].packageName); } ");

		scripts.add(newScript);

		newScript = new AgentScript();
		newScript
				.setSourceCode("agent.apps.launchAppByPackage('com.android.contacts');");
	...
	private ArrayList<AgentScript> lookupAgentsInFolder() {
		...
		for (File agentScriptFile : rootDir.listFiles(jsFilter)) {
			AgentScript newScript = new AgentScript();
